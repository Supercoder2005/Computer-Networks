...
int main(){
    ...
    scanf("%d",&n);
    int data[n][SIZE];
    printf("\nEnter the databits of %d %d-bit groups : \n",n,SIZE);
    for(i=0;i<n;i++){
        for(j=0;j<SIZE;j++){
            scanf("%d",&data[i][j]);
        }
    }
    // perform binary addition between each groups of databits
    int sum[SIZE] = {0};
    for(i=0;i<n;i++){
        binaryAddition(sum,data[i],sum);
    }
    // calculate the checksum by complementing the binary addition sum using 1's complement
    int checksum[SIZE] = {0};
    for(i=0;i<SIZE;i++){
        checksum[i] = 1-sum[i];
    }
